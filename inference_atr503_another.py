...
ATR_DIR = 'atr503/full'
OUT_DIR = 'wav/atr503_another'  # special directory
FEATURE_DIR = '/work/abelab4/s_koha/s_koha_work/exp/averaged_feature'
...
def main():
    ...
    # 各条件名を取得（F-student_convertedなど）
    dir_list = [f for f in os.listdir(FEATURE_DIR) if os.path.isdir(os.path.join(FEATURE_DIR, f))]
    for cname in dir_list:
        print(f'Condition: {cname}')

        for emotion in emotion_list:
            print(f'Emotion: {emotion}')
            save_dir = os.path.join(dname, cname, emotion)
            ...
            if use_embed or use_embed_ssl:
                ...
                feature_path = os.path.join(FEATURE_DIR, cname, feature_name, f'{emotion}.pt')
