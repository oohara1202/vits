...
from utils import load_wav_to_torch, load_filepaths_and_text, load_filepaths_and_text_without_sid
...
class TextAudioEmbedLoader(torch.utils.data.Dataset):
    """
        ...
        3) computes spectrograms from audio files.
    """
    def __init__(self, files_path, hparams):
        # multi-speakerの場合にsidを抜く
        self.audiopaths_and_text = load_filepaths_and_text_without_sid(files_path)
...
class TextAudioEmbedCollate():
    ...
    def __call__(self, batch):
        ...
        spec_lengths = torch.LongTensor(len(batch))
        wav_lengths = torch.LongTensor(len(batch))
        embeds = torch.rand(len(batch), len(batch[0][3]))  # for embedding conditioning, [B, embed_dim]
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            embeds[i] = row[3]  # for embedding conditioning, [embed_dim]
...
class TextAudioSSLEmbedCollate():
    """ Zero-pads model inputs and targets
    """
    def __init__(self, return_ids=False):
        self.return_ids = return_ids
    ...
    def __call__(self, batch):
        ...
        if self.return_ids:
            return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, embeds_ssl_padded, embeds_ssl_lengths, ids_sorted_decreasing
        return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, embeds_ssl_padded, embeds_ssl_lengths
